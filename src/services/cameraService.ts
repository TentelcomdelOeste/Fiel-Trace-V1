...
export const cameraService = {
  ...
  async _saveToNativeGallery(dataUrl: string, fileName: string): Promise<boolean> {
    try {
      ...
      // 1. Intentar obtener el identificador del álbum 'Field Trace' si ya existe en MediaStore
      try {
        const albumsRes = await Media.getAlbums();
        const existingAlbum = albumsRes?.albums?.find((a: any) => a.name === 'Field Trace');
        if (existingAlbum?.identifier) {
          albumIdentifier = existingAlbum.identifier;
        }
      } catch (albumError) {
        console.warn('[Galería] Error consultando álbumes existentes:', albumError);
      }

      // 2. Si no se encontró en MediaStore, asegurar la creación del directorio del álbum
      if (!albumIdentifier) {
        try {
          await Media.createAlbum({ name: 'Field Trace' });
        } catch (createError) {
          console.info('[Galería] Nota al crear álbum Field Trace (puede que el directorio ya exista):', createError);
        }

        // Obtener la ruta base de álbumes de Android para definir el identificador del directorio
        try {
          const albumsPathRes = await Media.getAlbumsPath();
          if (albumsPathRes?.path) {
            albumIdentifier = `${albumsPathRes.path}/Field Trace`;
          }
        } catch (pathError) {
          console.warn('[Galería] Error obteniendo ruta base de álbumes:', pathError);
        }
      }

      // 3. Verificación secundaria si aún no tenemos identificador
      if (!albumIdentifier) {
        try {
          const recheck = await Media.getAlbums();
          const found = recheck?.albums?.find((a: any) => a.name === 'Field Trace');
          if (found?.identifier) {
            albumIdentifier = found.identifier;
          } else if (recheck?.albums && recheck.albums.length > 0) {
            albumIdentifier = recheck.albums[0].identifier;
          }
        } catch (e) {
          console.warn('[Galería] Error en verificación secundaria de álbum:', e);
        }
      }

      // 4. Guardar la fotografía usando Media.savePhoto con un identificador de álbum válido
      ...
      console.log('[Galería] ✓ Fotografía guardada exitosamente en la galería:', result);
      return true;
    } catch (error: any) {
      console.error('[Galería] ✗ Error en primer intento de guardado:', error?.code || error?.message || error);

      // Intento de respaldo directo por si la plataforma no requiere identificador explícito
      try {
        const baseName = fileName.replace(/\.[^.]+$/, '');
        const fallbackResult = await Media.savePhoto({
          path: dataUrl,
          fileName: baseName
        });
        console.log('[Galería] ✓ Fotografía guardada en modo de respaldo:', fallbackResult);
        return true;
      } catch (fallbackError: any) {
        console.error('[Galería] ✗ Error guardando fotografía en la galería nativa:', fallbackError?.code || fallbackError?.message || fallbackError);
        return false;
      }
    }
  },
